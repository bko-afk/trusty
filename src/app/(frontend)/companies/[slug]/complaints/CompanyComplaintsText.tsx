...
export type CompanyComplaintItem = {
  id: string
  authorName: string
  title: string
  body: string
...
export function CompanyComplaintsPanel({ slug, companyName, complaints }: ComplaintsPanelProps) {
  ...
  return (
    <section aria-labelledby="company-complaints-heading">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <h2 id="company-complaints-heading" className="text-3xl font-extrabold">{text.title} «{companyName}»</h2>
      ...
      <div className="space-y-5">
        {complaints.map((complaint) => (
          <article key={complaint.id} className="border border-gray-200 bg-white p-6">
            ...
            <h3 className="mt-4 text-xl font-extrabold">{complaint.title}</h3><p className="mt-3 whitespace-pre-line leading-7 text-gray-600">{complaint.body}</p>
...
export function CompanyComplaintsText(props: ComplaintsPanelProps) {
  const { t, locale } = useLanguage()
  return (
    <div className="container-page py-8">
      <Breadcrumbs items={[{ label: t.common.home, href: '/' }, { label: t.nav.catalog, href: '/companies' }, { label: props.companyName, href: `/companies/${props.slug}` }, { label: copy[locale].breadcrumb }]} />
      <CompanyComplaintsPanel {...props} />
    </div>
  )
}
